...
func ChangePasswordByOld(ctx context.Context, verifyToken string, uid string, newPwd string, newSalt string) error {
	user, err := repository.GetUserByID(uid)
	if err != nil {
		return err
	}
	isValid, err := checkLoginToken(ctx, user, verifyToken)
	if err != nil {
		return err
	}
	if !isValid {
